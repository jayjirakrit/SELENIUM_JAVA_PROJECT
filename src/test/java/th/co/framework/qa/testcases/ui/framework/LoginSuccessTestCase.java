...
import th.co.framework.qa.ui.page.framework.SecurePages;
...
public class LoginSuccessTestCase extends BaseWeb {
    ...
    public void constructor(ITestContext context) {
        ...
        this.securePage = new SecurePages(driver, wait);
    }

    @Test(priority = 1)
    public void LoginSuccessTest() throws Exception {
        String testCaseId = "login_success_id_0001";
        driver.get(baseUrlUi + "/login");
        LoginModels model = new Gson().fromJson(String.valueOf(GetJsonDataUi.userLoginSuccessTestCase().get(testCaseId)), LoginModels.class);
        ...
        wait.until(ExpectedConditions.visibilityOfAllElements(securePage.message));
        new Common().screenCaptureFromBase64(childReport, driver);
        //Validate Login Successful
        assertThat(securePage.getMessage()).contains(model.expectedResultKey.get(0));
    }

    @Test(priority = 1, dependsOnMethods = {"LoginSuccessTest"})
    public void LogoutSuccessTest() throws Exception {
        String testCaseId = "logout_success_id_0001";
        LoginModels model = new Gson().fromJson(String.valueOf(GetJsonDataUi.userLoginSuccessTestCase().get(testCaseId)), LoginModels.class);
        parentReport = extent.createTest(model.scenarioName, model.scenarioName);
        childReport = parentReport.createNode(model.description);
        new Common().addLogs(context, model.description, Arrays.toString(model.expectedResult.toArray()));
